...
class Object:
    def __init__(self, x, y, height, width, mass, color, image = 0):
        ...
        if image != 0:
            ...
            self.img_x = self.x + (self.width / 2)
            self.img_y = self.y + (self.height / 2)
...
def calc_gravity(m1, m2, r, o1, o2, speed):
    ...
    a = F / m2
    s = a * t * t
    distance = round((((o1.x - o1.width) - (o2.x + o2.width)) * (pixel_meter / 300)) / 1000, 2)
...
objects.append(Object(1800, wn_size[1] / 2, 100, 600, 5.972 * 10e24, (0, 0, 255), pygame.image.load('Erth.png')))

objects.append(Object(0, (wn_size[1] / 2) - 10, 20, 20, 60, (0, 255, 0)))

speed = 1
last_time = time.time()

while run:  
    print((last_time - time.time()) * -1)
    ...
    data = calc_gravity(m1 = objects[0].mass , m2 = objects[1].mass, r =  pixel_meter * (objects[0].x - (objects[1].x + (objects[1].width / 2))), o1 = objects[0], o2 = objects[1], speed = speed)
    clock.tick(FPS)
    text = []
    pos = 0
    for i in range(0, 4):
